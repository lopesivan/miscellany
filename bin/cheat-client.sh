...
if [ $LISTOPT -eq 1 ]; then
	echo 'Exemoplo para gs: cht.sh -o gs'
	echo 'cheatsheets/iprj'
	echo 'Exemoplo para iprj: cht.sh iprj'
	echo
	echo 'cht.sh '${1-commando}
	echo 'cht.sh cheat.sheets:'${1-commando}
	echo 'cht.sh cheat:'${1-commando}
	echo 'cht.sh tldr:'${1-commando}
	echo 'cht.sh learnxiny:'${1-commando}
	echo 'cht.sh question:'${1-commando}
	echo
	exit 0
fi
...
# IVAN
#CHTSH_URL=http://192.168.2.43:8002
...
if [ "$CHTSH_MODE" = auto ] && [ -d "$CHEATSH_INSTALLATION" ]; then
	curl() {
		...
		url="$1"
		shift
		PYENV_VERSION=ve PYTHONIOENCODING=UTF-8 "python" "$CHEATSH_INSTALLATION/lib/standalone.py" "${url#"$CHTSH_URL"}" "$@"
	}
